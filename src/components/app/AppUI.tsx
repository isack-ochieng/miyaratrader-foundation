...
export function PageHeader({ eyebrow, title, subtitle, className, action }: { eyebrow?: string; title: string; subtitle?: string; className?: string; action?: ReactNode }) {
  return (
    <div className={cn("flex flex-wrap items-end justify-between gap-4", className)}>
      <div>
        {eyebrow && <div className="text-xs uppercase tracking-widest text-primary mb-2">{eyebrow}</div>}
...
export function ComingSoon({ title, description, icon: Icon }: { title: string; description: string; icon: React.ComponentType<{className?:string}> }) {
  return (
    <div className="glass rounded-2xl p-8 flex flex-col items-center text-center relative overflow-hidden min-h-64 justify-center">
      ...
      <div className="text-[10px] font-semibold uppercase tracking-widest rounded-full bg-primary/20 text-primary px-2 py-1 mb-3">Coming in Phase 2</div>
