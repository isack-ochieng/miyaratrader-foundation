...
interface Settings { user_id: string; theme: string; language: string; email_notifications: boolean; push_notifications: boolean; marketing_emails: boolean; two_factor_enabled: boolean }
...
function SettingsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setS((data as Settings) ?? { user_id: u.user.id, theme: "dark", language: "en", email_notifications: true, push_notifications: true, marketing_emails: false, two_factor_enabled: false });
    })();
  }, []);
  ...
  if (!s) return <PageContainer><div className="p-8 flex items-center gap-2 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin"/> Loading settings…</div></PageContainer>;
