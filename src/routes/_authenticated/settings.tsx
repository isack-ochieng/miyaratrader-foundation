...
function SettingsPage() {
  ...
  if (!s) return <PageContainer><div className="flex items-center gap-2 p-8 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" /> Loading settings…</div></PageContainer>;
    ...
    <PageContainer>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <Select value={s.theme} onValueChange={(v) => setS({ ...s, theme: v })}>
                ...
                <SelectContent><SelectItem value="dark">Dark</SelectItem><SelectItem value="light" disabled>Light (coming soon)</SelectItem></SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select value={s.language} onValueChange={(v) => setS({ ...s, language: v })}>
                <SelectTrigger className="mt-1.5 w-full max-w-xs"><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="en">English</SelectItem><SelectItem value="si" disabled>Sinhala (unavailable)</SelectItem><SelectItem value="ta" disabled>Tamil (unavailable)</SelectItem><SelectItem value="es" disabled>Spanish (unavailable)</SelectItem></SelectContent>
              </Select>
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <SettingsRow label="Two-factor authentication" desc="Extra layer of protection at sign-in (coming soon)" checked={s.two_factor_enabled} onChange={(v) => setS({ ...s, two_factor_enabled: v })} disabled />
        </Card>
      </div>
...
function SettingsRow({ label, desc, checked, onChange, disabled }: { label: string; desc: string; checked: boolean; onChange: (v: boolean) => void; disabled?: boolean }) {
  return <div className="flex items-center justify-between gap-4 border-b border-border/60 py-3 last:border-0"><div className="min-w-0"><div className="text-sm font-medium">{label}</div><div className="text-xs leading-5 text-muted-foreground">{desc}</div></div><Switch checked={checked} onCheckedChange={onChange} disabled={disabled} /></div>;
}
