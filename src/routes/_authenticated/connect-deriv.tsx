...
function ConnectDerivPage() {
  ...
  async function refresh() {
    ...
    const { data } = await supabase
      ...
      .select("id,account_id,currency,balance,status,is_virtual,last_synced_at")
  ...
  return (
    <PageContainer>
      <PageHeader
        eyebrow="Deriv connection"
        title="Deriv accounts"
        subtitle="Linked account records and connection status"
      />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-semibold flex items-center gap-2">
                      {c.account_id}{" "}
                      <span className="text-[10px] uppercase tracking-widest rounded-full bg-muted px-2 py-0.5 text-muted-foreground">
                        {c.is_virtual ? "Demo" : "Real"}
                      </span>
                    </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <ol className="mt-4 text-sm space-y-2 text-muted-foreground">
            ...
            <li className="flex gap-2">
              <span className="text-primary">2.</span> Log in to Deriv and approve access
            </li>
            <li className="flex gap-2">
      ...
      {!loading && connections.length === 0 && (
        <Card>
          <div className="text-sm text-muted-foreground flex items-center gap-2">
            <XCircle className="h-4 w-4" /> No accounts connected yet.
          </div>
        </Card>
      )}
