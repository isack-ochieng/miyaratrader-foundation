...
export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "MiyaraTrader account overview and saved connection details.",
      },
    ...
    (async () => {
      ...
      const [{ data: connData }, { data: profData }] = await Promise.all([
        supabase
          ...
          .select("account_id,currency,balance,status,is_virtual")
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="md:col-span-2 relative overflow-hidden">
          ...
          <h2 className="mt-2 text-2xl font-display font-semibold">
            {conn ? "Deriv account" : "No Deriv account linked"}
          </h2>
          ...
          <div className="mt-6 flex gap-3">
            {conn ? (
              <Button asChild>
                <Link to="/trading">
                  Open trading <ArrowRight className="ml-1 h-4 w-4" />
                </Link>
              </Button>
            ) : (
              <Button asChild>
                <Link to="/connect-deriv">
                  Connect Deriv <Link2 className="ml-1 h-4 w-4" />
                </Link>
              </Button>
            )}
        ...
        <Card>
          ...
          <div className="mt-3 flex items-center gap-3">
            <span
              className={`h-2.5 w-2.5 rounded-full ${conn ? "bg-primary shadow-[0_0_0_4px_oklch(0.72_0.16_160/0.2)]" : "bg-muted-foreground/60"}`}
            />
      ...
      <div className="grid gap-4 md:grid-cols-3 mt-4">
        <Card>
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <TrendingUp className="h-4 w-4 text-primary" /> Trading summary
          </div>
          <div className="mt-3 text-3xl font-semibold">
            {conn ? `${conn.balance?.toFixed?.(2) ?? "—"}` : "—"}
            <span className="text-sm text-muted-foreground ml-1">{conn?.currency ?? ""}</span>
          </div>
        ...
        <Card>
          ...
          <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
            <li className="flex gap-2">
              <CheckCircle2 className="h-4 w-4 text-primary shrink-0" /> Account created
            </li>
            {conn && (
              <li className="flex gap-2">
                <CheckCircle2 className="h-4 w-4 text-primary shrink-0" /> Deriv account connected
              </li>
            )}
            <li className="flex gap-2 opacity-60">
              <Clock className="h-4 w-4 shrink-0" /> No further activity recorded
            </li>
          </ul>
        </Card>
        <Card>
          ...
          <div className="mt-1 text-xs text-muted-foreground">
            This release supports account management and connection records.
          </div>
        </Card>
      </div>
        ...
        {future.map((f) => (
          <Card key={f.title} className="opacity-90">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-widest rounded-full bg-muted px-2 py-1 text-muted-foreground">
                Coming soon
              </span>
            </div>
...
const future = [
  {
    ...
    body: "Courses, mentorship, and structured learning paths.",
  },
  {
    icon: Bot,
    title: "Trading Bots",
    body: "Automate strategies and let bots trade on your behalf.",
  },
...
function QuickAction({
  ...
  return (
    <Link
      to={to}
      className="glass rounded-xl p-4 flex items-center gap-3 hover:border-primary/40 transition"
    >
      <span className="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-primary/15 text-primary">
