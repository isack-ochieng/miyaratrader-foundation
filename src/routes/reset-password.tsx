    ...
    <div className="min-h-screen bg-hero flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="glass rounded-2xl p-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">Enter a new password to secure your account.</p>
          {!ready && <div className="mt-4 text-xs text-muted-foreground">Waiting for recovery link… If nothing happens, request a new reset email.</div>}
          <form onSubmit={submit} className="space-y-4 mt-6">
            <div><Label>New password</Label><Input type="password" required minLength={6} value={password} onChange={e=>setPassword(e.target.value)} className="h-11 mt-1.5"/></div>
            <Button type="submit" className="w-full h-11" disabled={loading || !ready}>{loading ? "Updating…" : "Update password"}</Button>
          </form>
        </div>
      </div>
    </div>
